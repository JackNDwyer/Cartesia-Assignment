...
### Idea: As the Line and Ink products mature, create a new homepage that touches on all of these models (assuming strategically Cartesia wants to push those).
...
### Idea: Make the menu button more clear or add simple navigation to docs/pricing. The ? suggests support, but not necessarily docs/pricing.
...
### Idea: Startup Plan = 20 Agents concurrency != 5 TTS concurrency. I understand the AI isn't always talking, but these numbers should be closer.
...
Idea: A testimonial or more use cases/functionality could be useful here. Right now it's just another announcement about Sonic 3.
...
### Idea: The banner can be shrunk to one line of text. It currently spans more than two.
...
### Idea: Have an example prompt library that can easily be scrolled through—or pick a set of standard example prompts. Allow selecting other example prompts without needing to delete the filled text.
...
### Idea: When I change models, remove the emotion tags, or at least have the TTS model ignore them if incompatible and provide a warning: "model not compatible with emotion tags."
...
### Idea: The emoji buttons on the `Text to Speech` page should add the text tag in the text box. Right now users can add a text tag or use the emoji. If Cartesia wants to train users on text tags, tags should always be shown in the text box.
...
### Idea: Order the `All` voices so it’s understandable for users, and ensure the `RECENTLY USED` section in `All` reflects recently used cloned voices. On refresh, it didn't update.
...
### Idea: Add a link to the cloning page in the empty state so users can create their own voice if they want. Same situation for "Starred." Instead of an empty state, give a CTA to `Star` favorite voices for easy access. This already exists for the empty state of `My Voices` on the `Voices` page.
...
### Idea: Remove the "Copy Link" option and simply have a button to "Copy ID" at the top level for every voice.
...
### Idea: The sidebar can be consolidated to `Voice`, `Line`, and `Platform`. `Voice` can contain everything it currently does plus `Pronunciation`. The `Voices` and `Text to Speech` pages can be consolidated (the primary value to users is finding a voice they like and generating snippets), and `Narration`, unless highly utilized, seems redundant and tries to compete with ElevenLabs.
...
### Idea: Users will want a warm start with Text-to-agent. It doesn't provide much value currently, but with tool-calling and code editing it could. Generally, tool-calling feels like a big opportunity.
...
### Idea: For FTUX, maybe pre-seed new accounts with a starter agent. The current text-to-agent doesn't provide much value, so having users go through these steps is low ROI. Another idea is to nudge them toward a template. Instead of just saying "empty" when there are no agents, provide a CTA.
...
### Idea: Link to the GitHub repo from the Cartesia dashboard.
...
I also would have liked a realtime demo on the Agents page. Voice agents are a significant part of the business, and latency is a key differentiator. Let devs experience Line/the latency first hand so they’re comfortable rolling it out to their customers.

Pricing was initially overwhelming, but once I focused on my use case it made sense. The mismatch between concurrency for TTS and Agents was the only thing that seemed off. I understand why TTS concurrency is lower (GPU capacity planning), but as a dev, I can’t utilize additional agents without TTS.
...
There was considerable friction there.

Metrics I only briefly looked at. They seem okay for an MVP. As long as users can get an aggregate view of call outcomes, it’s in a good spot.

The three things I would change:
1. The homepage is the first touchpoint to frame the offering and build trust. If Cartesia is to be a one-stop Voice Agent platform (as I was told), we should talk more about Line/Ink and present it as a comprehensive solution. TTS is overrepresented since the landing page is /sonic. If Line is to be pushed, we should add a demo so devs can try a voice agent. Now, devs must go through several steps to make one themselves before trying.
2. Consolidating the sidebar would clean up the playground. As mentioned, there’s redundancy across Text-to-speech, Voices, and Narration. There’s also a voice-specific page that conveys the voice name, ID, language, etc. I don’t believe this adds value, and if anything, creates more cognitive overhead for users we want generating snippets and building agents.
3. Text-to-agents should allow for tool calls. Voice agents exist to replace humans who talk **and** do things. As I understand it, the Text-to-agents feature generates Voice Agents that can talk but not do things. While this is a bigger lift, since agents/tool calls are code-based, until then I suspect devs find this feature lacking.

Part 2: (30 min)
- MKT, Weekly: Website visits, broken down by UTM parameter. Checked weekly. Useful for monitoring traffic sources and which channels are working. 
...
- ACT, Weekly: API activations (2+ successful calls) – are users successfully running code with Cartesia's API? This signals strong intent.
...
3. Bring metrics to the team. If there are TVs, display the main metrics. During meetings, tie asks and outcomes to metric deltas.
...
How much lift can you expect?
If needed, note your intuition-based estimate of that metric's baseline.

My experiments would be:
## 1: Create an email drip campaign for unengaged users (from the suggested templates)

Many users sign up and churn at various stages for a variety of reasons. The product has information on user sign-up time and in-app activity, and there are clues in the data as to why users churn. I have had success with campaigns where emails are triggered based on a combination of user product activity and timing.

For example, if 48 hrs pass after signup and no action is taken, send a series of emails showing off the product, explaining free credits, and other information that could get users to try the product.

The core metrics impacted are resurrections (get people back for new features) and activations (convince new sign-ups to try the product).

Lift depends on what the user last did. If they were active/paying and then left, they might have gone out of business or found another solution.

My estimate: about 40% of sign-ups activate, and only 20% of those (so about 8% of sign-ups) convert to paying. With campaigns, both metrics should improve: more sign-ups will activate (probably +10%), and churned users will get a nudge, increasing the chance to try again or check out new features.

## 2: Allow the Text-to-agent product in Line to generate code, including tool calls, logic, transfers, and metrics.

Currently, Text-to-agent supports creating only a single-prompt voice agent. My belief is that without tool-calling (and logic, transfers, metrics), these agents are not useful for most business scenarios, which I suspect are most of Line’s use cases.

I’d add features for Text-to-agent to generate tool calls, logic, transfers, and relevant metrics. This helps builders realize their vision more quickly, and rather than acting as an AI middle-man via Cursor, Cartesia’s AI (with product context) would generate the agent. I still think agents should be code-based, but connecting Git could become optional if agents are manageable via the UI playground.

The main metric this impacts is activations on the Line product. More agents could be built—and, ideally, deployed to production—more easily. 
